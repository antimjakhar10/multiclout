...
function HelpCenter() {
  ...
  useEffect(() => {
    axios.get(`${API}/site-settings`).then((res) => {
      if (res.data.success) {
        setData(res.data.settings.helpCenter);
      }
    });
  }, []);
        ...
        <div className="border-b border-slate-200 bg-slate-50">
          <div className="mx-auto w-full max-w-[1320px] px-4 py-14 sm:px-6 lg:px-8">
            ...
            <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">
              Find answers, support information, and useful guidance related to Multiclout services.
            </p>
          </div>
        </div>

        <div className="mx-auto w-full max-w-[1320px] px-4 py-10 sm:px-6 md:py-12 lg:px-8">
                ...
                {contentBlocks.map((block, index) => {
                  ...
                  if (block.type === "ol") {
                    return (
                      <ol
                        key={index}
                        className="list-decimal space-y-2 pl-6 text-[16px] leading-8 text-slate-700 marker:font-semibold marker:text-[#167a7a]"
                      >
                        {block.items.map((item, itemIndex) => (
                          <li key={itemIndex}>{item}</li>
                        ))}
