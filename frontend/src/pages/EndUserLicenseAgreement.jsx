...
function EndUserLicenseAgreement() {
  ...
  useEffect(() => {
    axios.get(`${API}/site-settings`).then((res) => {
      if (res.data.success) {
        setData(res.data.settings.endUserLicenseAgreement);
      }
    });
  }, []);
        ...
        <div className="border-b border-slate-200 bg-slate-50">
          <div className="mx-auto w-full max-w-[1380px] px-4 py-14 sm:px-6 lg:px-8">
            ...
            <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">
              Please read this license agreement carefully before using our
              services or content.
            </p>
          </div>
        </div>
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8 lg:p-10">
            {contentBlocks.length === 0 ? (
              <p className="text-base text-slate-500">
                End User License Agreement content not available.
              </p>
            ) : (
